...
The output `mod$beta` is a `nrow(mod$idx)`-by-`length(mod$lambda)` matrix. Each column of `mod$beta` is a vector of estimate of all variable coefficients considered in Step 3 corresponding to one value of the lasso tuning parameter `lambda`. For example,
```{r}
