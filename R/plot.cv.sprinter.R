...
plot.cv.sprinter <- function(fit) {
  ...
  lambda <- fit$lambda[[fit$num_keep_best]]
  lam <- -log(lambda)

  cvm <- fit$cvm[fit$num_keep_best, ]
  cvse <- fit$cvse[fit$num_keep_best, ]
  cvup <- cvm + cvse
  cvlo <- cvm - cvse
  ...
  nzm <- fit$fit$step3[[fit$num_keep_best]]$nzm
  nzi <- fit$fit$step3[[fit$num_keep_best]]$nzi

  axis(side = 3, at = lam, labels = paste(paste(nzm), "/", paste(nzi)),
       tick = FALSE, line = 0, col.axis = "Orange")
  legend("topright", paste0("With ", fit$num_keep[fit$num_keep_best], " candidate interactions after Step 2."), bty = "n", pch = NA, cex = 1.2)

  lam_min <- lambda[fit$lam_best]
  lam_1se <- lambda[fit$lam_1se]
  abline(v = -log(lam_min), lty = 2, col = "Blue")
  abline(v = -log(lam_1se), lty = 2, col = "Green")
}
