#' Summary function of sprinter fit
#'
...
#' summary(fit)
#'
#' # summarize the fit from cv.sprinter, only looking that the first 70 lambdas
#' fit.cv <- cv.sprinter(x = x, y = y)
#' summary(fit.cv$fit, which = fit.cv$num_keep_best, index = 1:70)
#'
#' @export
summary.sprinter <- function (fit, which = 1, label = TRUE, index = NULL)
{
  p <- fit$p
  ...
  label_i <- fit$step3[[which]]$idx[(p + 1):pp, 1:2]
